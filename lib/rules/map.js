    ...
    create(context) {
        return {
            'CallExpression[callee.type="MemberExpression"][callee.object.name="_"][callee.property.name="map"]'(node) {
                ...
                if (node.parent && node.parent.type === NODE_TYPES.ASSIGNMENT_EXPRESSION) {
                    const closesStatement = findClosestStatement(node).parent;
                    const statementParent = closesStatement.parent;

                    if (statementParent.type === NODE_TYPES.IF_STATEMENT
                        && isArrayMethod(statementParent.test.callee)) return;
                }
